...
<body>
    ...
    $sql = "SELECT * FROM quiz_QAs LIMIT 30";
    $result = mysqli_query($conn,$sql);
    if (mysqli_num_rows($result) > 0) {
        if($_GET['id'] >= 21 && $_GET['id'] <= 31){
            $id = $_GET['id'];
        }else{
            $id=21;
        }
        ...
        while($row=mysqli_fetch_assoc($result)){
               if($id==$row['id']){
                ...
                if(isset($_POST['submit'])){
                    ...
                    echo '<div id="options">';
                        echo '<div><input type="radio" id="opt1" name="js" value="opt1"';
                        if($chk==true && $radio=='opt1'){echo'checked';}
                        echo'>';echo $row['opt1']; echo'</div>';
                        echo '<div><input type="radio" id="opt2" name="js" value="opt2"';
                        if($chk==true && $radio=='opt2'){echo'checked';}
                        echo'>';echo $row['opt2'];echo'</div>';
                        echo '<div><input type="radio" id="opt3" name="js" value="opt3"';
                        if($chk==true && $radio=='opt3'){echo'checked';}
                        echo'>';echo $row['opt3'];echo'</div>';
                        echo '<div><input type="radio" id="opt4" name="js" value="opt4"';
                    ...
                    echo '<button  id="update"><a href="js.php?id=';echo $id+1;echo '">';
